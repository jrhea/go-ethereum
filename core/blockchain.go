...
func (bc *BlockChain) ProcessBlock(ctx context.Context, parentRoot common.Hash, block *types.Block, config ExecuteConfig) (result *blockProcessingResult, blockEndErr error) {
	...
	if bc.cfg.NoPrefetch || !ok {
		...
		go func(start time.Time, throwaway *state.StateDB, block *types.Block) {
			...
			vmCfg.Tracer = nil
			bc.prefetcher.Prefetch(block, throwaway, bc.jumpDestCache, bc.precompileCache, vmCfg, &interrupt)
	...
	res, err := bc.processor.Process(pctx, block, statedb, bc.jumpDestCache, bc.precompileCache, bc.cfg.VmConfig)
	spanEnd(&err)
	if err != nil {
